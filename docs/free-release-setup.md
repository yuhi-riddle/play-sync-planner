...
- 定期通知は、GASの無料時間主導トリガーで約1時間おきに実行できます。日程回答を送った直後の通知はすぐ作られます。
...
Vercel Hobbyは毎時Cronを使えないため、GASから既存の通知APIを約1時間おきに呼び出します。設定は [GASで1時間ごとに通知を実行する手順](./gas-notification-schedule.md) を使います。

- 日程回答を送った直後の主催者向け通知はすぐ作られる
- 未回答、清算、支払い待ち、回答期限の定期確認はGASが約1時間おきに行う
