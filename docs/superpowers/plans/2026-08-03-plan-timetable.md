...
| 論点 | 決定 | 理由 |
|---|---|---|
| 終了時刻が無い**最後の**行の「いまここ」 | **その日のうちは**光り続ける（JST の日付が変わったら消える） | 終了時刻を決める材料が無い。当日ページを開いて何も光らないより、最後の項目が残るほうが自然。ただし日付で切らないと、8/15 17:00「解散」が翌 8/16 の朝まで光り続ける（Task 4 のレビューで発覚。当初は「開始したら光り続ける」と書いていた）。副作用として、23:00「二次会」→ 翌 01:00「解散」のような深夜またぎでは 00:30 に何も光らない。`groupTimetableItemsByDate` が開始日で切っているのと一貫しているので、割り切りとして受け入れる |
