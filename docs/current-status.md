...
- イベント作成、編集、一覧、詳細
- イベント一覧の進行中・下書き・中止・完了フィルター、カテゴリ・表示順・10/20/50件表示
- イベント一覧カードで具体的な状態、日時、場所、参加人数を確認
- 日程が終了・中止になっても清算が残るイベントは「進行中」に残し、カードでは「清算待ち」と表示。日程と清算の両方が終わったイベントだけを「完了」に表示
- 複数の日程調整があるイベントは、今後いちばん近い確定日時を一覧へ表示し、すべての日程が終わるまで完了扱いにしない
- 中止時に清算が始まっていないイベントは「中止」へ移し、清算中または未清算のものだけを「進行中」に残して「清算待ち」と表示
