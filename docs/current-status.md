...
- イベント一覧カードで確定日時、場所、参加人数、日程調整数、清算状況、次の対応を確認
- 日程が終了・中止になっても清算が残るイベントは「対応中」に残し、日程と清算の両方が終わったイベントだけを「完了」に表示
- 複数の日程調整があるイベントは、今後いちばん近い確定日時を一覧へ表示し、すべての日程が終わるまで完了扱いにしない
- 中止時に清算が始まっていないイベントは「中止」へ移し、清算中または未清算のものだけを「対応中」に残す
