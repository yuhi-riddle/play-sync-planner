...
1. 設計資料を更新する
   - `README.md` と現状整理は更新済みです。
   - 元の設計資料は Phase 1 時点の資料として残し、現在仕様は `docs/current-status.md` と `docs/release-checklist.md` に集約します。
...
3. 画面文言と導線を見直す
   - 利用規約とプライバシーポリシーは、通知、清算、支払いURLの扱いを反映済みです。
   - 残りは実ブラウザ確認で違和感が出た箇所だけ直します。
...
1. Supabase に `009_site_notifications.sql` まで適用する。
2. `docs/release-checklist.md` に沿って、ローカル確認を進める。
3. ユーザーがブラウザ確認を指示したタイミングで、実ブラウザ確認を行う。
4. チェックリストで見つかった問題だけを直す。
