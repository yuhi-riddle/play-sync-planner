-- list_owned_event_ids の並び順「開催日が近い順」(soonest) を直す。
--
-- 今までは schedule_start の単純な昇順だった。status=active には
-- 「開催済みだが清算が終わっていない」イベント(settlement_waiting)も含まれるため、
-- 数ヶ月前の未清算イベントが、来週に迫った予定より前に並んでしまっていた
-- （Codexレビュー指摘。PR #42 で並び順の既定を soonest にした直後に発覚）。
--
-- 直し方: 未来と過去でバケツを分ける。
--   1. 未来の予定がある(schedule_start >= now) → 先に、昇順（近い順）
--   2. 過去だけ(未清算等) → そのあとに、降順（直近に終わったものほど優先度が高い）
--   3. 日付が無い → 最後
-- newest / latest はこの問題が無いので触らない。引数・戻り値は変わらないので
-- create or replace のみでよい（drop不要）。
    ...
    select
      ...
      row_number() over (
        order by
          ...
          -- soonest: 未来(schedule_start >= now)を優先バケツにする。false(未来)が true(過去)より先、
          -- null(日付なし)は最後。この1本だけで「未来バケツ→過去バケツ→日付なし」の3段が決まる。
          case when sort_value = 'soonest' then (schedule_start < now()) end asc nulls last,
          -- 未来バケツの中だけ昇順（近い順）。過去バケツの行はこのキーが null になり、
          -- 上のバケツキーで既に分離済みなので影響しない。
          case when sort_value = 'soonest' and schedule_start >= now() then schedule_start end asc,
          -- 過去バケツの中だけ降順（直近に終わったものを優先）。
