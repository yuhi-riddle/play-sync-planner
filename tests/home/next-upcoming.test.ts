...
describe("jstStartOfToday", () => {
  it("JST の当日 0 時（= 前日 15:00 UTC）を返す", () => {
    // now = 2026-09-02 09:00 JST。当日 0 時は 2026-09-01 15:00 UTC。
    expect(jstStartOfToday(now).toISOString()).toBe("2026-09-01T15:00:00.000Z");
  });

  it("UTC で日付が変わる時間帯でも JST 基準で判定する", () => {
    // 2026-09-02 00:30 JST（= 2026-09-01 15:30 UTC）。JST ではもう 9/2。
    const earlyMorningJst = new Date("2026-09-02T00:30:00+09:00");
    expect(jstStartOfToday(earlyMorningJst).toISOString()).toBe("2026-09-01T15:00:00.000Z");
  });
});
