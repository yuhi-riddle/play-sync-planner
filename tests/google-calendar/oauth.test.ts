...
describe("google calendar oauth", () => {
  it("builds an OAuth URL with offline access and Calendar events readonly scope", () => {
    ...
    expect(url.searchParams.get("scope")).toBe(CALENDAR_EVENTS_READONLY_SCOPE);
  ...
  it("refreshes an access token", async () => {
    ...
    const fetchImpl = vi.fn(async () => ({
      ok: true,
      json: async () => ({ access_token: "new-access", expires_in: 3600, scope: CALENDAR_EVENTS_READONLY_SCOPE })
    })) as unknown as typeof fetch;
