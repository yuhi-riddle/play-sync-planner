...
export default function PlanErrorPage({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error(error);
  }, [error]);
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-control border border-line bg-surface p-5 shadow-soft backdrop-blur">
        ...
        <h1 className="mt-2 text-3xl font-bold tracking-normal text-ink sm:text-4xl">日程調整を表示できませんでした</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted">
          一時的な通信エラーか、ページの読み込みに失敗した可能性があります。もう一度試すか、日程調整一覧に戻ってください。
        </p>
        {error.digest ? (
