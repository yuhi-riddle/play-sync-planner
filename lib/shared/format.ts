 ...
 *
 * 予定・候補の日時を表示するところは基本これを使う。日付見出しが近くにあっても、
 * 行ごとに日付を出すなら曜日も出す（見出しの日付と行の日付が食い違って見えないように）。
 * ホームの「次の予定」カード、イベント一覧カードの確定日時、plan の確定日時カード、
 * 候補ランキング／確定フォームの候補カード、選択日の予定一覧、日程調整カレンダーの
 * タイムライン、いずれもこれ。
 *
 * 曜日なしの formatDateTimeRange は、時刻だけ／日付だけを出すような限定的な場面に留める。
 */
