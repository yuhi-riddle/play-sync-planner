        ...
        <ul className="mt-3 grid gap-2">
          {busyRanges.map((busyRange) => (
            <li
              ...
              className="rounded-lg border border-ink/8 bg-cream/72 px-3 py-2 text-sm text-ink"
            >
              <p className="font-bold">
                {formatTime(busyRange.start)} - {formatTime(busyRange.end)}
              </p>
              <p className="mt-1 font-bold">{busyRange.title || "予定あり"}</p>
              {busyRange.location ? <p className="mt-1 text-xs text-ink/60">場所: {busyRange.location}</p> : null}
            </li>
          ))}
        </ul>
      ) : null}
      {!loading && !error && busyRanges.length > 0 ? (
        <p className="mt-3 text-xs leading-5 text-ink/55">予定名と場所だけ表示しています。詳細や参加者は取得しません。</p>
      ) : null}
      {conflict ? (
