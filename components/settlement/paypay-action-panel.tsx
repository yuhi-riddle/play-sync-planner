...
export function PayPayActionPanel({
  ...
  return (
    <div className={`rounded-control border border-moss/18 bg-mist/28 p-3 ${className}`}>
      <p className="text-eyebrow uppercase text-pine">PayPay</p>
      <p className="mt-1 text-sm leading-6 text-muted">金額をコピーしてから、PayPayアプリで支払います。</p>
      <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
        ...
        <button
          type="button"
          onClick={openPayPay}
          className="inline-flex min-h-11 w-full items-center justify-center rounded-full bg-gradient-to-br from-pine to-pine-deep px-4 py-2 text-sm font-bold text-white shadow-soft transition-colors hover:from-pine-deep hover:to-pine-deep focus:outline-none focus:ring-2 focus:ring-clay focus:ring-offset-2 sm:w-auto"
        >
          <ExternalLink aria-hidden="true" className="mr-2 h-4 w-4" />
          PayPayを開く
        </button>
      </div>
      <p className="mt-2 text-xs leading-5 text-muted">スマホのPayPayアプリがある場合に開きます。</p>
    </div>
  );
}
